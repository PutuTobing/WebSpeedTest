...
// HTML escaping to prevent XSS when rendering server-supplied data into innerHTML
...
async function loadUserServers() {
    ...
    serverList.innerHTML = servers.map(server => `
        <div class="server-card ${escapeHtml(server.status)}">
            <div class="server-card-header">
                <h4>${escapeHtml(server.companyName)}</h4>
                <span class="status-badge status-${escapeHtml(server.status)}">${getStatusText(server.status)}</span>
            </div>
            <div class="server-card-body">
                <div class="server-info">
                    <span class="info-icon">📍</span>
                    <span><strong>Lokasi:</strong> ${escapeHtml(server.location)}</span>
                </div>
                <div class="server-info">
                    <span class="info-icon">🌐</span>
                    <span><strong>URL:</strong> ${escapeHtml(server.serverUrl)}</span>
                </div>
                <div class="server-info">
                    <span class="info-icon">⚡</span>
                    <span><strong>Bandwidth:</strong> ${escapeHtml(server.bandwidthCapacity)}</span>
                </div>
                <div class="server-info">
                    <span class="info-icon">💻</span>
                    <span><strong>Spec:</strong> ${escapeHtml(server.cpuCores)} Cores / ${escapeHtml(server.ramSize)} GB RAM</span>
                </div>
                <div class="server-info">
                    <span class="info-icon">👤</span>
                    <span><strong>PIC:</strong> ${escapeHtml(server.contactName)} (${escapeHtml(server.contactEmail)})</span>
                </div>
                ${server.companyWebsite ? `
                <div class="server-info">
                    <span class="info-icon">🌐</span>
                    <span><strong>Website:</strong> <a href="${escapeHtml(server.companyWebsite)}" target="_blank" rel="noopener noreferrer">${escapeHtml(server.companyWebsite)}</a></span>
                </div>
                ` : ''}
                ${server.additionalNotes ? `
                <div class="server-info">
                    <span class="info-icon">📝</span>
                    <span><strong>Catatan:</strong> ${escapeHtml(server.additionalNotes)}</span>
                </div>
                ` : ''}
            </div>
            <div class="server-card-footer">
                ...
                <div class="server-actions">
                    <button class="btn-edit" onclick="editServer('${escapeHtml(server.id)}')">✏️ Edit</button>
                    <button class="btn-delete" onclick="deleteServerConfirm('${escapeHtml(server.id)}')">🗑️ Hapus</button>
                </div>
            </div>
        </div>
    `).join('');
